...
export function fromCallData(rawAmount: BigNumberish, scale: number): BigNumber {
	return new BigNumber(rawAmount.toString()).shiftedBy(-scale);
}

export function toCalldata(displayAmount: BigNumber | string, scale: number = 18): BigNumber {
...
export function displayCurrency(value: string | number | BigNumber): string {
	...
	if (abs.isGreaterThanOrEqualTo(1_000_000_000)) {
		...
		formatted = bn.toFixed(significantDigits).replace(/\.?0+$/, '');
	}

	return formatted.trimEnd() + suffix;
}
