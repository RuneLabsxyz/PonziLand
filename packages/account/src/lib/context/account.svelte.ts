...
async function scanObjectForWalletsCustom(): Promise<void> {
	...
	availableWallets = await Promise.all(
		wallets.map(async (wallet: WALLET_API.StarknetWindowObject) => {
			...
			if (!isValid) {
				try {
					// eslint-disable-next-line @typescript-eslint/no-explicit-any -- Don't really have a choice there
...
const checkCompatibility = async (myWalletSWO: WALLET_API.StarknetWindowObject) => {
	...
	try {
		const _ = (await myWalletSWO.request({
...
export class AccountManager {
	...
	private async setup(): Promise<AccountManager> {
		const previousWallet: string | null = localStorage.getItem(previousWalletSymbol.toString());
	...
	private getSessionFromStorage() {
		...
		// eslint-disable-next-line svelte/prefer-svelte-reactivity -- False positive
