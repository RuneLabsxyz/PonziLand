...
impl AvnuPriceProvider {
    ...
    async fn fetch_avnu_prices_debug(
        ...
        for (chunk_idx, chunk) in token_addresses.chunks(MAX_TOKENS_PER_REQUEST).enumerate() {
            ...
            println!("Request {}: Query params: {:?}", chunk_idx + 1, query_params);
            ...
            println!("Request {}: Response body: {}", chunk_idx + 1, response_text);

            let chunk_prices: Vec<TokenPriceDto> = serde_json::from_str(&response_text).map_err(|e| {
                println!("Request {}: JSON parsing error: {}", chunk_idx + 1, e);
                Error::JsonParse(format!("{}", e))
            })?;

            println!("Request {}: Parsed {} token prices", chunk_idx + 1, chunk_prices.len());
        ...
        println!("Total parsed {} token prices across all requests", all_prices.len());
        Ok(all_prices)
    }
}
...
mod tests {
    ...
    async fn test_paging_with_many_tokens() {
        ...
        let reference_token = "0x049d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7";
        let tokens = vec![
            "0x001".to_string(), "0x002".to_string(), "0x003".to_string(), "0x004".to_string(),
            "0x005".to_string(), "0x006".to_string(), "0x007".to_string(), "0x008".to_string(),
            "0x009".to_string(), "0x00a".to_string(), "0x00b".to_string(), "0x00c".to_string(),
        ];

        let result = provider.get_price_of_pairs(reference_token, tokens.clone()).await;
        ...
        assert!((actual_ratio - 1.0).abs() < 0.00001, "Token 0x001 ratio should be 1.0");
        ...
        assert!((actual_ratio - 12.0).abs() < 0.00001, "Token 0x00c ratio should be 12.0");
    }
