...
impl From<f64> for U256FD128 {
    fn from(value: f64) -> Self {
        ...
        if value.is_nan() {
            panic!("Cannot convert NaN to U256FD128");
        }
        if value.is_infinite() {
            panic!("Cannot convert infinite value to U256FD128");
        }
        ...
        if abs_value >= 2.0f64.powi(127) {
            panic!("Value too large to convert to U256FD128");
        }
        ...
        result = result | U256::from(frac_u128);
