...
mod tests {
    ...
    #[test]
    fn test_normalize_text_substitutions() {
        // Verify normalization catches common substitutions
        let normalized = normalize_text("n1gg3r");
        assert!(normalized.contains("nigger"));

        let normalized = normalize_text("f4gg0t");
        assert!(normalized.contains("faggot"));

        let normalized = normalize_text("r3t4rd");
        assert!(normalized.contains("retard"));
    }

    #[test]
    fn test_blocks_mixed_case_slurs() {
        assert!(check_message("NiGgEr").is_some());
        assert!(check_message("FaGgOt").is_some());
        assert!(check_message("RETARD").is_some());
        assert!(check_message("Chink").is_some());
    ...
    #[test]
    fn test_blocks_slurs_in_sentences() {
        assert!(check_message("you are a nigger").is_some());
        assert!(check_message("go away faggot lol").is_some());
        assert!(check_message("what a retard").is_some());
    }

    #[test]
    fn test_blocks_spaced_evasion() {
        // Spaces stripped for detection
        assert!(check_message("n i g g e r").is_some());
        assert!(check_message("f a g g o t").is_some());
    }

    #[test]
    fn test_allows_legitimate_words() {
        // Words that might share substrings with blocked words
        assert!(check_message("snigger").is_some()); // contains "nigger" substring - blocked intentionally
        assert!(check_message("vinegar").is_none());
        assert!(check_message("bigger").is_none());
        assert!(check_message("digger").is_none());
    }
    ...
    #[test]
    fn test_validate_message_with_whitespace_content() {
        // Only whitespace should fail
        assert!(validate_message("  ").is_err());
        assert!(validate_message("\t\n").is_err());
        // Content with leading/trailing whitespace should succeed
        assert!(validate_message("  hello  ").is_ok());
    }

    #[test]
    fn test_validate_message_at_boundaries() {
        // Exactly at min length (1 char after trim)
        assert!(validate_message("x").is_ok());
        // Exactly at max length
        assert!(validate_message(&"b".repeat(500)).is_ok());
        // One over max length
        assert!(validate_message(&"c".repeat(501)).is_err());
    }
}
