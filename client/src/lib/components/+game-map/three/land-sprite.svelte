<script lang="ts">
  ...
  let circlePositionsCache = new Map<string, Array<{ x: number; y: number; circle: number }>>();
  ...
  $effect(() => {
    if (accountState.address !== ownershipCache.address) {
      const ownedIndices = accountState.address ? store.getOwnedLandIndices(accountState.address) : [];
      ownershipCache = {
  ...
  let ownedCoinTiles = $derived.by(() => {
    if (!ownershipCache.address || !visibleLandTiles || ownershipCache.ownedIndicesSet.size === 0) return [];
