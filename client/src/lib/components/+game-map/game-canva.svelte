<script lang="ts">
  ...
  $effect(() => {
    ...
    if (isTutorial && TUTORIAL_CAMERA.lockControls) {
      lockCameraControls(gameStore.cameraControls);
    } else {
      applyDefaultCameraControls(
        gameStore.cameraControls,
        devsettings.cameraControlsLeftClick,
        devsettings.cameraControlsRightClick,
        devsettings.CameraControlsWheel,
      );
    }
  });
      ...
      <CameraControls
        ...
        oncreate={(ref: CameraControlsRef) => {
          const settings = getCameraSettings(
            isTutorial,
            TUTORIAL_CAMERA.centerX,
            TUTORIAL_CAMERA.centerY,
            TUTORIAL_CAMERA.zoomLevel,
            CENTER,
            100,
          );
          ref.setLookAt(
