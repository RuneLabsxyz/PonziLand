<script lang="ts">
  ...
  import { SetLandQuest, RemoveLandQuest, StartQuest, GetQuestToken, GetQuestScore, ClaimQuest, GetQuestEntryPrice } from '$lib/stores/quests.svelte';
  ...
  async function getQuestInfo() {
    ...
    if (questDetails_res && questDetails_res.length > 0) {
      ...
      if (player_quest_res && player_quest_res.length > 0) {
        ...
        is_quest_over = score_res[1] == 1 || (currentQuestGame.quest_type == "Minigame" && score >= Number(questDetails?.target_score));
        ...
        {:else}
          ...
          {:else}
            {#if isHighScore}
              ...
              {#if is_quest_over}
                {#if hasWon}
                  <div class="mb-2 p-2 text-center border rounded bg-green-500/10 border-green-500">
                    <p class="text-lg font-bold text-green-500">🎉 Quest Complete!</p>
                    <p class="text-xs opacity-75">You've achieved the target score!</p>
                  </div>
                  <Button
