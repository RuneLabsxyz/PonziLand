<script lang="ts">
  ...
  let chartData = $derived.by(() => {
    ...
    const sellAmount = sellAmountInBaseToken
      ? Number(sellAmountInBaseToken.rawValue())
      : 0;

    // Determine chart time range (max of nuke time or payback time)
    const nukeHours =
      nukeTimeSeconds > 0 && nukeTimeSeconds !== Infinity
        ? nukeTimeSeconds / 3600
        : 0;
    const paybackHours =
      paybackTimeSeconds > 0 && paybackTimeSeconds !== Infinity
        ? paybackTimeSeconds / 3600
        : 0;
  ...
  let chartConfig = $derived.by(() => {
    ...
    return {
      ...
      scales: {
        x: (time: number) =>
          padding.left +
          (time / maxTime) * (width - padding.left - padding.right),
        y: (profit: number) =>
          padding.top +
          ((paddedMaxProfit - profit) / (paddedMaxProfit - paddedMinProfit)) *
    ...
    if (sellAmountInBaseToken && chartData.length > 0) {
      ...
      for (let i = 0; i < chartData.length - 1; i++) {
        ...
        if (
          ...
          const ratio =
            Math.abs(current.sellProfit) /
            Math.abs(next.sellProfit - current.sellProfit);
          const crossingHours =
            current.time + ratio * (next.time - current.time);

          const isGoingPositive =
            current.sellProfit <= 0 && next.sellProfit > 0;
      ...
      sellCrossings.forEach((crossing, index) => {
        if (crossing.hours <= chartConfig.domain.time[1]) {
          markers.push({
            ...
            label:
              crossing.type === 'breakeven' ? 'Sell Break Even' : 'Sell Limit',
  ...
  <div class="w-full bg-gray-900 p-4 rounded-lg">
    ...
    >
      ...
      <path
        d={profitPath}
        fill="none"
        stroke={netYieldPerHourInBaseToken &&
        netYieldPerHourInBaseToken.rawValue().isNegative()
          ? '#ef4444'
          : '#22c55e'}
        stroke-width="2"
      />
