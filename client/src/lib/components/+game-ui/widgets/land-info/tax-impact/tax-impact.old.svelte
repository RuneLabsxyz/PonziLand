<script lang="ts">
  ...
  $effect(() => {
    const currentCount = neighbors.getNeighbors().length;
    nbNeighbors = currentCount;
    currentNeighborCount = currentCount;
  });
  ...
  $effect(() => {
    if (stakeAmountVal) {
      ...
      estimateNukeTime(land).then((time) => {
        ...
        // Calculate max nuke time based on current stake amount and max neighbors
        const currentBalance = land.stakeAmount || 0;
        let maxRemainingHours =
