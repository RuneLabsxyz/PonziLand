<script lang="ts">
  ...
  import {
    ...
    setHeatmapOpacity,
    setHeatmapParameter,
    setHeatmapShowLegend,
  } from './heatmap.store.svelte';
  ...
  let currentParamConfig = $derived(getCurrentParameterConfig());
...
<div class="flex flex-col gap-4 p-4 min-w-[320px] min-h-0 h-full">
  ...
  <div class="flex items-center justify-between">
    <Label class="font-ponzi-number stroke-3d-black">Enable Heatmap</Label>
    <Button onclick={() => setHeatmapEnabled(!heatmapStore.enabled)}>
  ...
  {#if heatmapStore.enabled}
    ...
    <div class="space-y-2">
      ...
      <Select
        onSelectedChange={(v) =>
          setHeatmapParameter(v?.value as HeatmapParameter)}
      >
    ...
    <div class="space-y-2">
      ...
      <Select
        onSelectedChange={(v) =>
          setHeatmapColorScheme(v?.value as HeatmapColorScheme)}
      >
        <SelectTrigger class="w-full" variant="swap">
