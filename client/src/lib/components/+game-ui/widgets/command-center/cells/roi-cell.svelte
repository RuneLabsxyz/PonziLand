<script lang="ts">
  ...
  function calculateDurationHours(startDate: string, endDate: string | null): number {
    try {
  ...
  const durationHours = $derived(calculateDurationHours(position.time_bought, position.close_date));
  ...
  const tokenFlowYieldPerHour = $derived.by(() => {
    if (!totalInflowBaseEquivalent || !totalOutflowBaseEquivalent || durationHours <= 0) return null;
  ...
  const investmentROIPerHour = $derived.by(() => {
    if (!buyCostBaseEquivalent || !tokenFlowYieldPerHour || buyCostBaseEquivalent.rawValue().isZero()) {
      return null;
    }
  ...
  <div class="flex items-start justify-end gap-1">
    <div class="flex flex-col items-end">
      ...
      {#if tokenFlowYieldPerHour}
        <div class={tokenFlowYieldPerHour.rawValue().isPositive() ? 'text-green-400' : 'text-red-400'}>
          {tokenFlowYieldPerHour.rawValue().isPositive() ? '+' : ''}{tokenFlowYieldPerHour.rawValue().toNumber().toFixed(3)}$/h
        </div>
      {:else}
      ...
      <!-- ROI per Hour -->
      {#if investmentROIPerHour !== null}
        <div class={`text-xs ${investmentROIPerHour > 0 ? 'text-green-300' : 'text-red-300'}`}>
          {investmentROIPerHour > 0 ? '+' : ''}{formatPercentage(investmentROIPerHour)}%/h
        </div>
      {:else}
