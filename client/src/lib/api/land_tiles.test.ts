...
// Mock dev settings
vi.mock('$lib/components/+game-map/three/utils/devsettings.store.svelte', () => ({
  devsettings: {
    minRandomUpdates: 1,
    maxRandomUpdates: 5,
    nukeRate: 0.1,
...
const TEST_OWNER = '0x05144466224fde5d648d6295a2fb6e7cd45f2ca3ede06196728026f12c84c9ff';
const TEST_LOCATION = coordinatesToLocation({ x: 5, y: 5 });

// Helper function to create a mock ParsedEntity
function createMockEntity(models: Partial<SchemaType['ponzi_land']>): ParsedEntity<SchemaType> {
  return {
    entityId: '0x123',
    models: {
...
describe('LandTileStore', () => {
  ...
  describe('Basic initialization', () => {
    ...
    it('should return undefined for out-of-bounds coordinates', () => {
      ...
      expect(landStore.getLand(100, 5)).toBeUndefined();
      expect(landStore.getLand(5, 100)).toBeUndefined();
    });
  });
  ...
  describe('Land entity updates', () => {
    ...
    it('should create BuildingLand with stake when receiving both land and stake models', () => {
      ...
      const land = get(landStore.getLand(5, 5)!);
      expect(land).toBeInstanceOf(BuildingLand);
      expect((land as BuildingLand).stakeAmount?.toBigint()).toBe(BigInt(stakeModel.amount));
    });
    ...
    it('should store pending stake when receiving only stake model for empty land', () => {
      ...
      expect(updatedLand.stakeAmount?.toBigint()).toBe(BigInt(stakeModel.amount));
    });

    it('should delete land when receiving empty land model', () => {
      ...
      // Now delete it with empty model
      const deleteEntity = createMockEntity({ Land: {} as Land });
      landStore.updateLand(deleteEntity);

      // Should be empty land now
  ...
  describe('Auction entity updates', () => {
    ...
    it('should update existing AuctionLand when receiving new auction data', () => {
      ...
      const landModel = createLandModel();
      const auctionModel = createAuctionModel({ start_price: '1000000000000000000' });
      const entity = createMockEntity({ 
        Land: landModel, 
        Auction: auctionModel 
      });
    ...
    it('should convert to BuildingLand when auction finishes', () => {
      // Create auction land
      const landModel = createLandModel();
      const auctionModel = createAuctionModel({ is_finished: false });
      const entity = createMockEntity({ 
      ...
      // Finish the auction
      const finishedAuctionModel = createAuctionModel({ 
        is_finished: true,
        sold_at_price: { Some: '1500000000000000000' } as any
      });
      const finishEntity = createMockEntity({ 
        Land: landModel,
        Auction: finishedAuctionModel 
      });
      landStore.updateLand(finishEntity);

      // Should still be auction land with finished status
      land = get(landStore.getLand(5, 5)!);
      expect(land).toBeInstanceOf(AuctionLand);
      expect((land as AuctionLand).isFinished).toBe(true);
    });
  });

  describe('Complex update sequences', () => {
    it('should handle sequence: empty -> stake -> land (applies pending stake)', () => {
      // Start with empty land
      let land = get(landStore.getLand(5, 5)!);
      expect(land).toBeInstanceOf(EmptyLand);

      // Receive stake update first
      const stakeModel = createLandStakeModel({ amount: '3000000000000000000' });
    ...
    it('should handle sequence: empty -> auction -> land (preserves auction)', () => {
      // Start with empty land
      let land = get(landStore.getLand(5, 5)!);
      expect(land).toBeInstanceOf(EmptyLand);
    ...
    it('should handle sequence: building -> auction -> land update (preserves auction)', () => {
    ...
    it('should handle auction with stake updates', () => {
      // Create auction land
      const landModel = createLandModel();
      const auctionModel = createAuctionModel();
      const entity = createMockEntity({ 
      ...
      // Update stake for auction land
      const stakeModel = createLandStakeModel({ amount: '4000000000000000000' });
      const stakeEntity = createMockEntity({ LandStake: stakeModel });
      landStore.updateLand(stakeEntity);

      // Should still be auction land but with updated stake
      land = get(landStore.getLand(5, 5)!) as AuctionLand;
      expect(land).toBeInstanceOf(AuctionLand);
      expect(land.stakeAmount?.toBigint()).toBe(BigInt(stakeModel.amount));
    });
    ...
    it('should handle owner change in land updates', () => {
      ...
      const newOwner = '0x07144466224fde5d648d6295a2fb6e7cd45f2ca3ede06196728026f12c84c9aa';
  ...
  describe('Edge cases', () => {
    ...
    it('should preserve stake when updating BuildingLand without new stake', () => {
      ...
      const landModel = createLandModel();
      const stakeModel = createLandStakeModel({ amount: '2000000000000000000' });
      const entity = createMockEntity({ 
        Land: landModel, 
        LandStake: stakeModel 
      });
