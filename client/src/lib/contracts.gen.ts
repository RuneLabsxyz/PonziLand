...
export function setupWorld(provider: DojoProvider) {
	...
	const build_config_setFullConfig_calldata = (config: models.Config): DojoCall => {
		return {
			contractName: "config",
	...
	const config_setFullConfig = async (snAccount: Account | AccountInterface, config: models.Config) => {
		try {
			return await provider.execute(
				snAccount,
				build_config_setFullConfig_calldata(config),
				"ponzi_land",
			);
		} catch (error) {
			console.error(error);
