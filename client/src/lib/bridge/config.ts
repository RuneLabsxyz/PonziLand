...
export async function initializeHyperlane() {
  ...
  const enrichedMetadata: ChainMap<ChainMetadata & { mailbox?: Address }> =
    Object.fromEntries(
  ...
  console.log(
    '🔍 Enriched metadata (with mailboxes):',
    Object.entries(enrichedMetadata)
      .filter(
        ([name]) =>
          name.toLowerCase().includes('solana') ||
          name.toLowerCase().includes('starknet'),
      )
      .map(([name, meta]) => ({
        name,
  ...
  const filteredRoutes = Object.entries(warpRoutes).filter(
    ([_routeId, route]) => {
      const allTokensSupported = route.tokens.every((token) => {
        const chainName = token.chainName.toLowerCase();
        return SUPPORTED_CHAINS.some((supported) =>
          chainName.includes(supported),
        );
      });
  ...
  console.log(
    '🔍 Filtered routes (only Solana + Starknet):',
  ...
  console.log(
    ...
    Array.from(new Set(filteredTokens.map((t) => t.symbol))),
  );
  ...
  const warpConfig = {
    ...
    options: Object.fromEntries(
      filteredRoutes.map(([id, route]) => [id, route.options]),
    ),
  };
