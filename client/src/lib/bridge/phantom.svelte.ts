...
class PhantomWalletStore {
	...
	async initialize() {
		...
		if (!provider) {
			this.state.error = 'Phantom wallet not found. Please install Phantom wallet extension.';
			return;
		}
	...
	async connect(options?: { forcePrompt?: boolean }) {
		...
		try {
			...
			this.state.error = err instanceof Error ? err.message : 'Failed to connect wallet';
