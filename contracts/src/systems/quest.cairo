...
pub trait IQuestSystems<T> {
    ...
    fn register_quest_game(
        ref self: T,
        world_address: ContractAddress,
        namespace: ByteArray,
        game_contract_name: ByteArray,
        settings_contract_name: ByteArray,
        settings_id: u32,
        target_score: u32,
...
pub mod quests {
    ...
    use ponzi_land::interfaces::quests::{
        IOneOnOne, IOneOnOneDispatcher, IOneOnOneDispatcherTrait, Status,
    };
    use ponzi_land::models::auction::Auction;
    use ponzi_land::models::land::Land;
    use ponzi_land::models::quest::{
        Quest, QuestCounter, QuestDetails, QuestDetailsCounter, QuestGame, QuestGameCounter,
        QuestType,
    };
    ...
    pub impl QuestSystemsImpl of super::IQuestSystems<ContractState> {
        fn set_land_quest(ref self: ContractState, land_location: u16, game_id: u64) {
            ...
            let success = create_quest(
                ref self, quest_game, land_location, 1000000000000000000, // 10 strk
                1, caller,
            );
        ...
        fn start_quest(ref self: ContractState, land_location: u16, player_name: felt252) -> u64 {
            ...
            assert!(
                quest_details.creator_address != player_address,
                "Player is the owner of the quest land",
            );

            let to_us =
                false; //TODO: this should be true for auction quests, which aren't implemented yet

            // no payments if in test mode
            if store.get_quest_rewards_enabled() {
                validate_and_execute_quest_payment(
                    ref self, quest_details, get_caller_address(), store, to_us,
                );
            }
            ...
            let minigame_world_dispatcher = IWorldDispatcher {
                contract_address: quest_game.world_address,
            };
            let mut minigame_world: WorldStorage = WorldStorageTrait::new(
                minigame_world_dispatcher, @quest_game.namespace,
            );

            let (game_token_address, _) = minigame_world
                .dns(@quest_game.game_contract_name)
                .unwrap();
        ...
        fn finish_quest(
            ...
            assert!(claimable, "Quest {} is not completed", quest_id);

            if store.get_quest_rewards_enabled() {
                //TODO: finalize reward logic. If transfering land then make sure to refund stake
                //and verify new sell price/stake
                // or if giving a % of the sell price then make sure to handle that and checking for
                // nuke and everything
        ...
        fn get_score(self: @ContractState, quest_id: u64) -> (u32, bool) {
            ...
            // Check the minigame type and handle accordingly 🐙 Different scoring systems for
        ...
        fn register_quest_game(
            ...
            quest_game_count.count += 1;
            world.write_model(@quest_game_count);
            let quest_game = @QuestGame {
                id: quest_game_count.count,
                world_address,
                namespace,
                game_contract_name,
    ...
    fn create_quest(
        ...
        //let (settings_address, _) =
        //minigame_world.dns(@quest_game.settings_contract_name).unwrap();
    ...
    fn validate_and_execute_quest_payment(
        ...
        let validation_result = self
            .payable
            .validate(store.get_main_currency(), caller, quest.entry_price);
        ...
        let recipient = if to_us {
            store.get_our_contract_for_auction()
        } else {
            quest.creator_address
        };
        self
            .payable
